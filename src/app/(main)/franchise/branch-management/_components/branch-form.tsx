...
import { Building2, Loader2, MapPin, PlusCircle, Trash2, Edit } from "lucide-react";
...
export function BranchForm({ branch, onSuccess }: BranchFormProps) {
  const { updateBranch } = useFranchiseBranchStore();
  ...
  const onBranchSubmit = async (data: BranchFormValues) => {
    if (!branch) return; // Should not happen in edit mode
    try {
  ...
  return (
    <div className="space-y-8">
      {/* Branch Details Form */}
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onBranchSubmit)} className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Branch Details</CardTitle>
              <CardDescription>Update the name and location for this branch.</CardDescription>
            </CardHeader>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                This will permanently delete the &quot;{service.name}&quot; service. This action cannot
                                be undone.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
